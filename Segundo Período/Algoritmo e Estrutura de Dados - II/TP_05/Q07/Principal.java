...
class Cell {
 ...
 class List {
    ...
    private Cell primeiro;
    private Cell ultimo;

    public List() {

        primeiro = new Cell(null);
        ultimo = primeiro;
    }
    ...
    public boolean pesquisar(String name) {
        ...
        for(Cell i = primeiro.next; i != null; i = i.next) {
            ...
            if(i.elemento.getName().compareTo(name) == 0) {
...
class Hash {
    ...
    public int h(String name) {
        ...
        for(char c : name.toCharArray()) asciiValue += c;
    ...
    boolean pesquisar(String name) {
        ...
        int pos = h(name);
        boolean result = table[pos].pesquisar(name);



        if(result == true)  System.out.println("Posicao: " + pos);
    ...
    public void inserir(Game elemento) {
        ...
        table[pos].inserirInicio(elemento);
    }
}
...
public class Principal{
  ...
  String search_Name = scr.nextLine();

  // Second part
  while(true) {

      // Identify END
      if(isFim(search_Name)) break;

      // ------------------------------------ //
      
      if(!item.pesquisar(search_Name)) System.out.println("NAO");

      
      search_Name = scr.nextLine();
  }
