...
char *corta(char *string, int position, int length) {
...
void lista_insereinicio(Game x) {
...
void lista_insere(Game x, int pos) {
...
int main()
{
   ...
   fp = fopen("/tmp/games.csv", "r");
    ...
    for(int i = 0; i < ops; i++) {
        ...
        char linha[100], gameId[100], pos[3];

        scanf(" %[^\n]", linha);
        strcpy(gameId, corta(linha, 4, strlen(linha)));
        ...
        if(linha[0] == 'I') {
            ...
            if(linha[1] == 'I') {
                ...
                game_read(&game, gameId);
                lista_insereinicio(game);
             
            }
            else if(linha[1] == 'F') {

                game_read(&game, gameId);
                lista_inserefim(game);
           
            }
            else if(linha[1] == '*') {

                strcpy(pos, corta(linha, 4, 2));
        
                strcpy(gameId, corta(linha, 7, strlen(linha)));

                game_read(&game, gameId);
                lista_insere(game, atoi(pos)); 
            }
        }
        else if(linha[0] == 'R') {
            
            if(linha[1] == 'I')  printf("(R) %s\n", lista_removeinicio().name);
            else if(linha[1] == 'F') printf("(R) %s\n", lista_removefim().name);
            else if(linha[1] == '*') {
                
                strcpy(pos, corta(linha, 4, 2));
                printf("(R) %s\n", lista_remove(atoi(gameId)).name);
            }
        }
    }
