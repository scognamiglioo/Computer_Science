...
DoubleCell* newDoubleCell(Game element) {
...
void list_print() {
    ...
    for(i = first -> next; i != NULL; i = i -> next) game_print(&(i -> element));
}
...
void quicksort(DoubleCell** array, int low, int high) {
    ...
    Game pivot = array[(low + high) / 2] -> element;

    while(i <= j) {

        if(strcmp(array[i] -> element.release_date, pivot.release_date) == 0) {

            while(strcmp(array[i] -> element.name, pivot.name) < 0) i++;
        }
        else {

            while(strcmp(array[i] -> element.release_date, pivot.release_date) < 0) i++;
        }

        if(strcmp(array[j] -> element.release_date, pivot.release_date) == 0) {

            while(strcmp(array[j] -> element.name, pivot.name) > 0) j--;
        }
        else {

            while(strcmp(array[j] -> element.release_date, pivot.release_date) > 0) j--;
        }
