...
int getMaior(game array[], int n) {
    game maior = array[0];

    for (int i = 0; i < n; i++) {
        if(maior.upvotes < array[i].upvotes){
            maior.upvotes = array[i].upvotes;
        }
    }
    return maior.upvotes;
}

void countingsort(game array[], int n) {
    ...
    for (int i = 0; i < n; count[(int)array[i].upvotes]++, i++);

    //Agora, o count[i] contem o numero de elemento menores ou iguais a i
    for(int i = 1; i < tamCount; count[i] += count[i-1], i++);

    //Ordenando
    for(int i = n-1; i >= 0; ordenado[count[(int)array[i].upvotes]-1] = array[i], count[(int)array[i].upvotes]--, i--);
